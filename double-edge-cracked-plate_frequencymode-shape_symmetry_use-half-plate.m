%rectangular plate, cracks are parallel to the upper and lower boundaries. 3 segments are used.
clear all
clc
format long
symm=1; %symmetry indicator, 0: symmetric, 1: antisymmetric
n=30; %the number of terms of admissible functions in each of the two coordinate directions
n2=n^2;
n3=2*n2;
a=0.32; 
b=2*0.32; %a, b are the plate dimensions in x and y directions
h=0.002; %thickness
c1=0.1*a; %length of the left crack
c2=0.1*a; %length of the right crack
a1=c1;
a2=a-c1-c2;
a3=c2;
b1=b/2;
...
%penalty terms for edges of each plate segment kp1 kr1(left edge) kp2 kr2(upper edge) kp3 kr3(right edge) kp4 kr4(lower edge)
	...
	for i=1:n
		...
		for nn=1:n
			for j=1:n
				...
				K(u,v)=K(u,v)+kcr12*KC2(m)/a1*KC2(nn)/a1*Fni00(i,j)*b1; %part of the rotational boundary condition between segment 1 and segment 2 (right boundary of segment 1)
				...
				K(u+n2,v+n2)=K(u+n2,v+n2)+kct12*KP1(m)*KP1(nn)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 2 and segment 1 (left boundary of segment 2)
				K(u+n2,v+n2)=K(u+n2,v+n2)+kcr12*KC1(m)/a2*KC1(nn)/a2*Fni00(i,j)*b1; %part of the rotational boundary condition between segment 2 and segment 1 (left boundary of segment 2)
				K(u+n2,v+n2)=K(u+n2,v+n2)+kct23*KP2(m)*KP2(nn)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 2 and segment 3 (right boundary of segment 2)
				K(u+n2,v+n2)=K(u+n2,v+n2)+kcr23*KC2(m)/a2*KC2(nn)/a2*Fni00(i,j)*b1; %part of the rotational boundary condition between segment 2 and segment 3 (right boundary of segment 2)
				...
				K(u+n2,v+n2)=K(u+n2,v+n2)+kp4*KP3(i)*KP3(j)*Emi00(m,nn)*a2; %the translational boundary condition for the lower edge of segment 2
				K(u+n2,v+n2)=K(u+n2,v+n2)+ks4*KS3(i)*KS3(j)*Emi00(m,nn)*a2; %the slip shear boundary condition for the lower edge of segment 2
				K(u+n2,v+n2)=K(u+n2,v+n2)+kr4*KC3(i)/b1*KC3(j)/b1*Emi00(m,nn)*a2; %the rotational boundary condition for the lower edge of segment 2
				%for K33
				...
				K(u+n3,v+n3)=K(u+n3,v+n3)+kct23*KP1(m)*KP1(nn)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 3 and segment 2 (left boundary of segment 3)
				K(u+n3,v+n3)=K(u+n3,v+n3)+kcr23*KC1(m)/a3*KC1(nn)/a3*Fni00(i,j)*b1; %part of the rotational boundary condition between segment 3 and segment 2 (left boundary of segment 3)
				...
				K(u+n3,v+n3)=K(u+n3,v+n3)+kp3*KP2(m)*KP2(nn)*Emi00(i,j)*b1; %the translational boundary condition for the right edge of segment 3
				K(u+n3,v+n3)=K(u+n3,v+n3)+kr3*KC2(m)/a3*KC2(nn)/a3*Emi00(i,j)*b1; %the rotational boundary condition for the right edge of segment 3
				%for K12
...
for MODE_NUMBER=1:5	
	for i=1:size(x_nodes,2)
		...
		elseif x>a1&&x<=a1+a2 %when the point is in the middle segment (segment 2)
			...
			EIGENVECTOR=real(X(n3+1:3*n2,MODE_NUMBER))';
			for j=1:size(y_nodes,2)
				y=y_nodes(j);
				SET_X(1)=1;
				SET_X(2)=(x-a1-a2)/a3;
